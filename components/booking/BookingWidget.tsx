...
import { formatPrice, formatDuration, type Service, type OpeningHours } from "@/types/database";
...
export function BookingWidget({
  ...
  const [dateISO, setDateISO] = useState<string>(toDateInputValue(new Date()));
  ...
  const maxDate = toDateInputValue(new Date(Date.now() + 1000 * 60 * 60 * 24 * 60));
  ...
  useEffect(() => {
    ...
    getBookedRanges(businessId, dateISO).then((ranges) => {
      if (cancelled) return;
      setSlots(generateAvailableSlots(dateISO, openingHours, service.duration_minutes, ranges));
      setLoadingSlots(false);
    });
    return () => {
      cancelled = true;
    };
  }, [businessId, dateISO, service, openingHours]);
    ...
    return (
      <Card className="p-8 text-center">
        ...
        <p className="mt-2 text-sm text-ink2">
          We've sent your request to the business. You'll hear back to confirm your
          appointment.
        </p>
      </Card>
    );
            ...
            {services.map((s) => (
              <label
                ...
                className={
                  ...
                  (serviceId === s.id ? "border-ledger bg-ledger/5" : "border-line hover:border-ink/30")
                }
              >
      ...
      {step === 2 && service && (
        <div className="space-y-4">
          ...
          <div>
            ...
            {loadingSlots ? (
              <p className="text-sm text-ink2">Loading available times…</p>
            ) : slots.length === 0 ? (
              <p className="text-sm text-ink2">No times available this day. Try another date.</p>
            ) : (
      ...
      {step === 3 && service && selectedSlot && (
        ...
        >
          <div className="rounded-md bg-ink/5 px-4 py-3 text-sm text-ink2">
            ...
            <p>
              {selectedSlot.toLocaleDateString([], { weekday: "long", month: "long", day: "numeric" })} at{" "}
