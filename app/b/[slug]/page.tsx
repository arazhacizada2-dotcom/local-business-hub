...
import { formatPrice, formatDuration, DAY_LABELS, DAY_ORDER } from "@/types/database";
...
export default async function PublicBusinessPage({ params }: { params: { slug: string } }) {
  ...
  // Awaited (not fire-and-forget) so the insert reliably completes before
  // the serverless function's response finishes.
    ...
    <main className="min-h-screen bg-paper">
      <header className="border-b border-line bg-white">
        <div className="container-page flex items-center justify-between py-5">
          <Link href="/" className="font-display text-base text-ink">Local Business Hub</Link>
        </div>
      </header>
      ...
      <section className="border-b border-line bg-white">
        <div className="container-page py-14">
          ...
          <h1 className="mt-3 font-display text-4xl text-ink">{business.name}</h1>
          {business.description && (
            <p className="mt-4 max-w-xl text-ink2">{business.description}</p>
          )}
              ...
              {services.map((s) => (
                <div key={s.id} className="flex items-center justify-between gap-6 py-4">
                  <div>
                    ...
                    <p className="mt-1 font-mono text-xs text-ink2">{formatDuration(s.duration_minutes)}</p>
                  </div>
                  <p className="shrink-0 font-mono text-ink">{formatPrice(s.price_cents)}</p>
                </div>
              ))}
            </div>
          )}
              ...
              return (
                <div
                  ...
                  className={
                    ...
                    (day === today ? "text-ink font-medium" : "text-ink2")
                  }
                >
